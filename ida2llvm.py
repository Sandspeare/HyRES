...
def lift_tif(tif: ida_typeinf.tinfo_t, width = -1) -> ir.Type:
    ...
    if tif.is_func():
        ...
        if child_tif.is_void():     # or child_tif.is_udt():
...
def storecast(src, dst, builder):
    ...
    if dst != None and dst.type != src.type.as_pointer():
        ...
        if isinstance(dedereference(dst), ir.AllocaInstr):
            return dst
        else:
...
def get_offset_to(builder: ir.IRBuilder, arg: ir.Value, off: int = 0) -> ir.Value:
    ...
    #     return typecast(arg, ir.IntType(8).as_pointer(), builder)
        ...
        castPtr = typecast(arg, ir.IntType(8).as_pointer(), builder)
        return builder.gep(castPtr, (ir.Constant(ir.IntType(32), off),))
...
def _lift_from_address(module: ir.Module, ea: int, typ: ir.Type):
    if isinstance(typ, ir.FunctionType):
        ...
        with builder.goto_entry_block():
            ...
            count = {}
            for i, lvar in enumerate(list(pfunc.lvars)): 
...
def undefine_offset(code):
    ...
    for key in matches:
        for i in [1, 2, 4, 8, 16]:
            offsets.add(int(key) * i)
            for j in range(i):
                offsets.add(int(key) * i + j)

def get_i8ptr():
    flag = False
    for func in idautils.Functions():
        for var in ida_hexrays.decompile(func).lvars:
            if var.tif.is_ptr() and var.tif.get_ptrarr_object().is_char():
                i8tif = var.tif
                flag = True
                break
        if flag:
...
def check_mem_access(pfunc, inst, i8tif, index):
    lvar = None
    if inst.opcode == 1:
        if inst.d.t == hr.mop_d:
            subinst = inst.d.d
            if subinst.l.t == hr.mop_l and subinst.r.t == hr.mop_n and subinst.r.nnn.value > 0:
                lvar = subinst.l.l
            elif subinst.r.t == hr.mop_l and subinst.l.t == hr.mop_n and subinst.l.nnn.value > 0:
                lvar = subinst.r.l
    else:
        if inst.r.t == hr.mop_d:
            subinst = inst.r.d
            if subinst.l.t == hr.mop_l and subinst.r.t == hr.mop_n and subinst.r.nnn.value > 0:
                lvar = subinst.l.l
            elif subinst.r.t == hr.mop_l and subinst.l.t == hr.mop_n and subinst.l.nnn.value > 0:
    ...
    if lvar != None:
        if not (lvar.var().tif.is_udt() or (lvar.var().tif.is_ptr() and lvar.var().tif.get_ptrarr_object().is_udt())) or ("mystruct" in lvar.var().tif.dstr()):
            if pfunc.entry_ea not in baseinfo:
                baseinfo[pfunc.entry_ea] = set()
            baseinfo[pfunc.entry_ea].add(lvar.var().name)

    if inst.l.t == hr.mop_d:
        analyze_inst(pfunc, inst.l.d, i8tif, index, True)

    if inst.r.t == hr.mop_d:
        analyze_inst(pfunc, inst.r.d, i8tif, index, True)

    if inst.d.t == hr.mop_d:
        analyze_inst(pfunc, inst.d.d, i8tif, index, True)


def analyze_inst(pfunc, inst, i8tif, index, is_load = False):
    if inst:
        if is_load and inst.opcode in [0x0C, 0x0D]:
            if inst.l.t == hr.mop_n and inst.l.nnn.value > 0:
                offsets.add(inst.l.nnn.value)
                offsets.add(inst.l.nnn.value * 2)
                offsets.add(inst.l.nnn.value * 4)
                offsets.add(inst.l.nnn.value * 8)

            if inst.r.t == hr.mop_n and inst.r.nnn.value > 0:
                offsets.add(inst.r.nnn.value)
                offsets.add(inst.r.nnn.value * 2)
                offsets.add(inst.r.nnn.value * 4)
                offsets.add(inst.r.nnn.value * 8)

            if inst.d.t == hr.mop_n and inst.d.nnn.value > 0:
                offsets.add(inst.d.nnn.value)
                offsets.add(inst.d.nnn.value * 2)
                offsets.add(inst.d.nnn.value * 4)
                offsets.add(inst.d.nnn.value * 8)

        if inst.opcode in [1, 2, 3]:
            check_mem_access(pfunc, inst, i8tif, index)
        
        if inst.l.t == hr.mop_d:
            analyze_inst(pfunc, inst.l.d, i8tif, index)

        if inst.r.t == hr.mop_d:
            analyze_inst(pfunc, inst.r.d, i8tif, index)

        if inst.d.t == hr.mop_d:
            analyze_inst(pfunc, inst.d.d, i8tif, index)

def get_base_pointer(pfunc, i8tif):
    index = []
    for bi in range(pfunc.mba.qty):
        blk = pfunc.mba.get_mblock(bi)
        minsn = blk.head
        while True:
            if minsn == None:
                break
            analyze_inst(pfunc, minsn, i8tif, index)      
            minsn = minsn.next
    return index
...
def set_structs():
    ...
    for func in idautils.Functions():
        ...
        if pfunc == None:
            continue
        ptext[func] = pfunc
        get_base_pointer(pfunc, i8tif)

    fields = { num for num in offsets if 0 <= num <= 10000 }

    for func in idautils.Functions():
        funcname = ida_name.get_name(func) 
        binaryinfo["funcaddr"][funcname] = hex(func)
        if func not in baseinfo:
            continue

        if hex(func) in label["function"]:
            binaryinfo["function"][funcname] = label["function"][hex(func)]
        ...
        count = {}
        for lvar in lvars:
            if lvar.name in baseinfo[func]:
                index += 1
                tifptr = create_structs(index, fields)
                lvar.set_lvar_type(tifptr)
                ChangeVariableType(func, lvar, tifptr)
        ...
        for lvar in lvars:
            ...
            if "mystruct" in lvar.tif.dstr():
                binaryinfo["vartype"][hex(func)][varname] = lvar.tif.dstr()

        if "->mem" in str(pfunc):
            binaryinfo["cpp"][hex(func)] = str(pfunc)

        ptext[func] = pfunc

    return binaryinfo

def create_structs(index, fields):
    new_struc = ida_struct.get_struc(ida_struct.get_struc_id(f"mystruct_{index}"))
    if new_struc == None:
        sid = ida_struct.add_struc(ida_idaapi.BADADDR, f"mystruct_{index}", False)
        new_struc = ida_struct.get_struc(sid)
        for j in fields:
            ida_struct.add_struc_member(new_struc, f"mem{index}_{j}", j, ida_bytes.FF_BYTE, None, 1)  

    tif = ida_typeinf.tinfo_t()
    ida_typeinf.guess_tinfo(tif, new_struc.id)
    tifptr = ida_typeinf.tinfo_t()
    tifptr.create_ptr(tif)
    return tifptr   

def create_mapping():
    mapping = {}
    for func in binaryinfo["vartype"]:
        for key in binaryinfo["vartype"][func]:
            identifyA = func + "_" + binaryinfo["vartype"][func][key].split(" ")[0]
            if key not in label["vartype"][func]:
                continue
            identifyB = label["vartype"][func][key]
            if identifyB not in mapping:
                mapping[identifyB] = []
            mapping[identifyB].append(identifyA)
    binaryinfo["mapping"] = mapping
...
if __name__ == "__main__":
    ...
    idc.auto_wait()
    output = idc.ARGV[1]
    label = json.load(open(f"{output}.label"))
    #########################################################################
    i8tif = get_i8ptr() # find a i8ptr type
    offsets = set(range(512))   # support offset
    binaryinfo = set_structs()
    create_mapping()
    #########################################################################
    bin2llvm = BIN2LLVMController()
    bin2llvm.initialize()
    # bin2llvm.insertFunctionAtEa(0x11056)
    bin2llvm.insertAllFunctions()
    #########################################################################
    bin2llvm.save_to_file(f"{output}.ll")
    json.dump(binaryinfo, open(f"{output}.info", "w"), indent=1)
    print("end")
    # idc.qexit(0)