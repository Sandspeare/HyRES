...
{
	vector<Function*> deletFuncList;
	while (1)
	{
		deletFuncList.clear();
		for (Function &F : *mod)
		{
			if (F.getNumUses() == 0 && F.getName().find("###") == string::npos)
			{
				deletFuncList.push_back(&F);
			}
		}
		if (deletFuncList.size() == 0)
		{
			break;
		}
		for (int i = 0; i < deletFuncList.size(); i++)
		{
			deletFuncList[i]->removeFromParent();
		}
	}
